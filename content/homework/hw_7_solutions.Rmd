...
for (i in 1:nboot) {
  ...
  z.sim.full <- arima.sim(model = list("ar" = arma.fit.full$coef["ar1"], 
                                       "sd" = sqrt(arma.fit.full$sigma2)), 
                          n = m) + X[1:m, ]%*%beta.full
  z.sim.part <- arima.sim(model = list("ar" = arma.fit.part$coef["ar1"], 
                                       "sd" = sqrt(arma.fit.part$sigma2)), 
                          n = m) + X[1:m, ]%*%beta.part
  
  broc$z.sim.part <- NA
  broc$z.sim.part[1:m] <- z.sim.part
  y.sim <- z.sim.part - X[1:m, ]%*%beta.part
  
  arma.fit.full.sim <- arima(z.sim.full[1:m], order = c(p, 0, q),
                          method = "ML", 
                         xreg = X[1:m, ], include.mean = FALSE)
  pred.full.sim <- predict(arma.fit.full.sim, n.ahead = length(y) - m,
                       newxreg = X[(m + 1):length(y), ])$pred
  arma.fit.part.sim <- arima(y.sim[1:m], order = c(p, 0, q),
                         method = "ML")
  pred.part.sim <- predict(arma.fit.part.sim, n.ahead = length(y) - m)$pred + 
    pred[(m + 1):length(y)]
...
\color{blue}
See the plot after (e). The approach used does affect the approximate standard errors of the predicted values, which makes sense because the approximate standard errors for approach ii. account for uncertainty about $\boldsymbol \beta$ whereas the standard errors for approach i. do not. The variability due to uncertainty about the parameter estimates appears to dominate the variability due to future values being unobserved.
